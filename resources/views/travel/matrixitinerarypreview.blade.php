...
<div style="margin-bottom: 0.5rem; @if(! $loop->first && $loop->count > 1) margin-top: 1.5rem; @endif">{{ $slice['origin']['city']['name'] }} ({{ $slice['origin']['code'] }}) to {{ $slice['destination']['city']['name'] }} ({{ $slice['destination']['code'] }}) on {{ \Carbon\Carbon::parse($slice['departure'])->format('D, M j, Y') }}</div>
...
<div style="display: grid; grid-template-columns: 5% 95%">
    ...
    <div style="display: grid; grid-template-columns: 50% 25% 25%;">
        <div style="display: block;"><strong>{{ $segment['origin']['city']['name'] }} ({{$segment['origin']['code']}}) to {{ $segment['destination']['city']['name'] }} ({{ $segment['destination']['code'] }})</strong> on {{ \Carbon\Carbon::parse($segment['departure'])->format('D, M j, Y') }}</div>
