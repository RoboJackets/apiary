...
You have been assigned to {{ $assignment->travel->name }}. Please complete the following items as soon as possible so that we can book travel arrangements for you.
...
@else
You have been assigned to {{ $assignment->travel->name }}. Please pay the ${{ intval($assignment->travel->fee_amount) }} travel fee as soon as possible{{ $assignment->travel->return_date > \Carbon\Carbon::now() ? ' so that we can book travel arrangements for you' : '' }}. You can pay online with a credit or debit card at {{ route('pay.travel') }}.
@endif
